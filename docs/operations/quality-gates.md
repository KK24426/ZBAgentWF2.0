...
- 已确认是授权写任务，读取并独立 checkpoint 用户修改（如存在）；
- 修改范围、不涉及范围和验收条件明确；
...
- 接口和验收条件已经由用户定义或批准；
- 覆盖主路径、失败路径和至少一个回归路径；
- bugfix 已检查同包调用点、直接调用方和共享状态；
- adapter 不复制领域规则；
- 适用时保留错误来源、所属包或组件、阶段、业务 ID、退出码、超时/取消和脱敏摘要；
- 资源释放和关键副作用可见。
...
- 日志、错误、事件和测试证据均须脱敏；仅记录获准的业务摘要，并对摘要限制长度，不记录原始参数、SQL 参数或结果集。
- 异常类型、完整堆栈、cause 和 suppressed 链在脱敏后保留，不因上述摘要限长而整体截断。完整异常链不构成记录秘密或原始业务数据的授权。
...
高风险或跨职责范围修改使用完整报告：结果、影响范围、用户决策依据、验证命令与结果、Plan/Result Review、风险、未覆盖事项、偏离记录、commit、branch、remote/upstream 和 push 状态。
