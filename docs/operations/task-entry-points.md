...
所有任务先读根 `AGENTS.md`；涉及具体目录时再读最近的 `AGENTS.md` 和 `REQUIREMENTS.md`。真实代码和 Maven 配置是最终依据。
...
| 公共接口、DTO、状态和错误语义 | `docs/contracts/module-ports.md`、`docs/contracts/open-issues.md`、目标模块源码公开出口和 `REQUIREMENTS.md` | 用户批准、调用方、兼容性、文档同步 |
| 功能编码 | `docs/AI_DEV_GUIDE.md`、目标模块 `AGENTS.md`、`REQUIREMENTS.md`、相关接口和测试 | 修改范围、验收、分层验证 |
| 测试 | 目标源码、现有测试、目标模块 `REQUIREMENTS.md`、`docs/operations/quality-gates.md` | 主路径、失败路径、回归路径 |
...
| 数据或外部副作用 | `docs/operations/asset-policy.md`、相关契约、目标模块规则 | 环境、配置来源、权限、脱敏、停止条件 |
