...
common.project.bean 保存 Project→Requirement→RequirementTask 的一对多数据结构；user.project.userif 和 user.agent.userif 保存用户批准的项目与异步执行声明，目前无业务实现。AgentChatImpl 仅实现 AgentChat，不继承 AgentBase。
