...
4. 定位包和职责范围读 [code-map/modules.md](./code-map/modules.md)、目标包 `AGENTS.md` 和[根需求台账](../REQUIREMENTS.md)。
...
- `code-map/`：包职责及文件定位，不复制源码。
- `decisions/`：长期技术决策及其取舍。
- `operations/`：本地开发、AI 协作、质量和敏感资产规则。
- `templates/review/`：所有程序修改的实施前后空白 review 模板，具体任务记录不填入模板；保存位置见 [Agent Workflow](./operations/agent-workflow.md)。
